...
class Crawler(object, metaclass=ProxyMetaclass):  # 继承元类
    ...
    def crawl_daili66(self, page_count=4):  # 抓取各个网站的免费代理，可以自行扩展
        """
        获取代理66
        :param page_count: 页码
        :return: 代理
        ...
        for url in urls:
            print('Crawling', url)
            html = get_page(url)  # 调用get_page方法发送代理网站的请求，返回请求内容
            if html:
                doc = pq(html)  # 调用PyQuery方法，生成Pyquery对象
